...
date: "Sep 2016; rev. Sep 9, 2019, Sep 17-21, 2020, Sep 20 2022"
urlcolor: blue
---
...
`for (package in c("dplyr","ggplot2","raster","sp","rgdal","maptools","rgeos","sf")) {`
...
uss <- readOGR(file.path(data_path,"cb_2021_us_state_20m.shp"))
...
# Set up a 1 by 2 layout for plots (but don't forget to switch back to a single plot layout below
par(mfrow=c(1,2))
# Plot Oregon
...
# This code uses the sf package.
summary(or1)
or1
or1.box <- st_as_sfc(st_bbox(or1))
plot(or1.box)
plot(or1[1], add=TRUE)
```
...
```{r, message=FALSE, results='hide',warning=FALSE}
mgmt_file <- file.path(data_path,"mgmt.zip")

if(! file.exists(mgmt_file)){
 download.file("https://nrimp.dfw.state.or.us/DataClearinghouse/default.aspx?pn=ViewFile&att=ODFW/ODFW_805_5_ODFW_wildlife_mgmt_units_shapefile.zip",dest=mgmt_file, mode="wb") 
}

unzip(mgmt_file, exdir = data_path)
mgmt<-readOGR(file.path(data_path,"ODFW_wildlife_mgmt_units.shp"))
mgmt
...
Use the `get_nlcd` command from `FedData` to extract the NLCD data for this management unit. `FedData` does the cropping automatically using a shapefile or raster for "template". If you run into errors you may need to check what the projection of hood_nlcd is, and see the NOTE below.
```{r, message=FALSE, results='hide',warning=FALSE}
?get_nlcd
...
```
**NOTE** If you have trouble with the `FedData` package, the code below will import the 2011 Oregon NLCD directly (54 MB); it may take a few mins. If you didn't have trouble with the `FedData` package extracting the NLCD data, skip to the section below starting with "Plot hood_nlcd with raster package which uses base R plotting:"
...
Plot hood_nlcd with `raster` package which uses base R plotting:
```{r, message=FALSE, results='hide',warning=FALSE}
raster::plot(hood_nlcd)
# Next we will add a legend.
# For more on adding legends with raster (and classifying), see: https://www.earthdatascience.org/courses/earth-analytics/lidar-raster-data-r/classify-raster/
# To add a legend you need to refer to the NLCD classes https://www.mrlc.gov/data/legends/national-land-cover-database-2016-nlcd2016-legend

# You can look at the color codes for the legend of the raster (tif) in R like this:
hood_nlcd@legend

# This is from the current pal_nlcd command on the FedData github site: https://github.com/cran/FedData/blob/master/R/NLCD_FUNCTIONS.R). I've updated the color codes below to match the hood_nlcd codings (both the "code" and the associated "class" and "description")
pal_nlcd <- function() {
  data.frame(
    class = c("water", "water",
    ...
    color = c("#466B9F", "#D1DEF8",
              "#DEC5C5", "#D99282", "#EB0000", "#AB0000",
              "#B3AC9F",
              "#68AB5F", "#1C5F2C", "#B5C58F",
              "#A58C30", "#CCB879",
...
```
Pull out the relevant land cover code and description for the legend. The first 2 lines of code below uses `dplyr` to filter those that match `hood_nlcd` land cover classes. 

Plot the `hood_nlcd` with raster package which uses base R plotting: `xpd=FALSE` is the default to clip plot to plotting region. Leave a little room at the bottom for the legend with `mai`;  c(bottom, left, top, right)); units in inches.

x and y in the `legend` command are relative to the plotted figure units (which are meters); adjust these for a new raster or your RStudio plotting window size.

Note that the legend is correct for the colors but the mapping to pixels in the plot itself is not. See if you can figure out how to fix this (This is optional for this lab).
```{r, message=FALSE, warning=FALSE, fig.width=6, fig.height=8}
NLCD <- as.matrix(table(raster::values(hood_nlcd)))
cols <- dplyr::filter(pal_nlcd(), code %in% row.names(NLCD))

# Plot
par(xpd = FALSE,mai = c(2.5, 0.5, 1, 1)) 
plot(hood_nlcd, legend=FALSE,axes=TRUE,box=FALSE,col=cols$color,
     xlab="Easting (m)", ylab="Northing (m)",
     main="Hood Management Area Land Cover") 
par(xpd = TRUE) # reset to limit plotting to figure region
legend(x = -13580000, y = 5650000, legend = cols$description, fill = cols$color, 
       ncol = 2,  cex = 0.7, inset = 0.9) 

# Add a scalebar with the raster package
?scalebar # units are in your raster units (m) but you can display in km
scalebar(10000, type="bar", divs = 2, label = c(0,50,100),below="km")
# Add a north arrow on your own.

# You may need to clear the graphic to reset the graphics settings if you are going to make more plots after this.
#graphics.off()
```
...
# Since hood_metrics_for is a tibble, we use this format for plotting (using ggplot2). Note that the y axis scale varies by plot when faceting by metric below. We use a log10 scale for the x axis to visualize the spread more effectively. Note that we tell the code below to use dplyr for the "select" function because this command is also used by the MASS package, which is loaded with R.
...
## OPTIONAL Homework in preparation for Lab 4:
An optional portion of Lab 4 will use GitHub - you can decide whether or not to use GitHub.  GitHub is a versioning program helpful for collaborating and tracking changes in code. You can think of it as a Google Docs version for tracking code changes (you can also use it with text). If you are familiar with GitHub, no need to go over the following, but if you are not familiar, work through the following tutorials [personally, I use the Desktop version but any version will work]:

If you would like to get some practice using GitHub with Lab 4, please complete this first short tutorial ahead of Lab 4:
