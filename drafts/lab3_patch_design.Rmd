...
This lab has 2 Parts. Part 1 involves computing patch metrics. Part 2 involves applying patch metrics to design a sampling scheme. You will need to hand in a single PDF produced from your R Markdown file with code, plots, and answers to Part 1 and Part 2 to D2L. When referring to the code below it may be more useful to use the .Rmd file linked above. You will need to be connected to the internet to complete this lab.

At the end of Lab 3 is Homework in preparation for Lab 4 (familiarizing yourself with GitHub). 

# Part 1: Patch Metrics 

WHAT YOU NEED TO HAND IN FOR PART 1: Answer *QUESTIONS 1&2* below, in your R Markdown pdf.

We will be using the R package, SDMTools to complete the analysis below. Note that another package, "landscapemetrics" also exists. Feel free to try it out:
https://r-spatialecology.github.io/landscapemetrics/ and https://cran.r-project.org/web/packages/landscapemetrics/index.html. Both packages provide similar metrics based on Fragstats. See how they compare here:
...
for (package in c("raster","rgdal","maptools","rgeos")) {
...
# The package landscapemetrics may require loading separately
...
# FedData should be installed from github adn requires data.table package first
...
# We can project these data to match the NLCD data coordinate reference system (Albers, also known as NAD83 / Albers NorthAm). Fed Data R package returns NLCD data in a different CRS.
...
# XXXXXXX
us.aea.proj <- CRS("+proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +k=1.0 +units=m +nadgrids=@null +wktext  +no_defs")
...
[![bigmapa2](bigmapa2.gif)](https://www.dfw.state.or.us/resources/hunting/big_game/units/images/bigmapa2.gif)
...
summary(mgmt) # Its CRS is lcc (Lambert Conformal Conic: https://proj.org/operations/projections/lcc.html), GRS80. We should transform it to match NLCD data from FedData.
mgmt1<-spTransform(mgmt, merc.proj)
...
## FYI if you had a raster of mgmt1 instead of a shapefile (e.g., mgmt1_raster) you wanted to clip to the extent of another raster (e.g., hood_raster), you could do the following:

# hood_raster <- crop(mgmt1_raster, extent(hood))
# hood_raster <- mask(x = hood_raster, mask = hood)

# Use the get_nlcd command from FedData to extract the NLCD data for this management unit. FedData does the cropping automatically using a shapefile or raster for "template". 
?get_nlcd
...
###QUESTION 1: 
How do the 12 patch metrics from `landscapemetrics` help you understand how an organism interacts with its environment? For 3 metrics of your choice, explain in 1-2 sentences what the metric tells us about an organism's interaction with its environment. Feel free to use examples.
