...
*[RMarkdown version of this Lab](lab6.Rmd)*

This lab covers connectivity analysis with the `grainscape` R package. We have also included some extra OPTIONAL exercises at the end of the lab on interpolation with the `gstat` R package used in lab 5.

For the connectivity analysis, you need to hand in a **single PDF**, including output plots, any statistics, code, and text for answers to the questions. 
...
spc.rich <- spc.rich[-109] # spc.rich[109] has NA values in coordinates, we will consider only columns with valid coordinates
...
# Make points a matrix for coordinates by referencing  "east" and and "north" columns.
names(rs)
...
rs_grid = spsample(rs1, type = "regular", cellsize = c(100,100))
