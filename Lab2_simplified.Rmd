...
---
  ...
  *[RMarkdown version of this Lab](lab2_simplied.Rmd)*
...
dir<-"/Users/rassoul/Documents/GitHub/MSUGradSpatialEcology"
...
Add some point data for a species from GBIF.org. Get GBIF data with function gbif() from the *dismo* R package or choose your own favorite organism (in the US). Check the GBIF site first to see how many occurrences you'll be importing http://www.gbif.org/occurrence/search - enter your species name in the search window. Click on "filter" and scientific name. 
eg. Import the occurrence data on Puma concolor 

puma.gbif <- dismo::gbif("Puma", "concolor", geo=T, nrecs=300) 

I am using 'nrecs = 300' here to cut down on download time. To answer the lab questions, it would be better to use all the available records for the species you choose.

Puma concolor has 26,600+ records, and may take a few mins to import. Choose a different species in the US with a smaller number if you don't want to wait so long. It might take time to import the occurence data on puma through gbif. Alternatively, and for the purpose of this lab only, you can download the same data avalaible in the <a href="https://github.com/SpaCE-Lab-MSU/MSUGradSpatialEcology/tree/master/docs">"data/lab2" folder on GitHub</a> and store it in your data_path. This data was last updated on September 2025.
...
puma.gbif <- readr::read_delim(file.path(dir,"data/lab2/puma_concolor.csv"), delim = "\t", escape_double = FALSE,trim_ws =TRUE)

# plot occurrences
par(mfcol=c(1,1))
plot(us)
points(puma.gbif$decimalLatitude, puma.gbif$decimalLongitude,
...
puma <- SpatialPoints(coords=puma1[,c(23,22)], proj4string=wgs1984.proj)
...
puma <- SpatialPoints(coords=puma1[,c("decimalLongitude","decimalLatitude")], proj4string=wgs1984.proj)
...
summary(us.equal) 
#What is the dimention of the geomoetry?
head(us.equal$geometry)
#The spatial data contains Z (3D)we need to remove the z dimension before creating the raster
??st_zm
...
us.raster <- raster::rasterize(us.equal, r, getCover=TRUE)
...
puma.raster <- raster::rasterize(puma.equal, r,getCover=TRUE)
...
usr <- sf::read_sf(file.path(data_path,"cb_2014_us_nation_20m.shp"))
## take a look at the shapefile info
summary(usr)
