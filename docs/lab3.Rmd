...
This lab has 2 Parts. Part 1 involves computing patch metrics. Part 2 involves applying patch metrics to design a sampling scheme. You will need to hand in a **single PDF** produced from your R Markdown file with code, plots, and answers to Questions 1-3, to D2L. When referring to the code below it may be more useful to use the .Rmd file linked above. You will need to be connected to the internet to complete this lab.
...
lcc.proj<-terra::crs("+proj=lcc +lat_0=41.75 +lon_0=-120.5 +lat_1=43 +lat_2=45.5 +x_0=400000 +y_0=0 +datum=NAD83 +units=ft +no_defs")
...
hood.merc<-sf::st_transform(hood, merc.proj)
...
plot(hood.merc,add=FALSE, lwd=2, border='white')
```
...
# landscapemetrics::check_landscape(hood_nlcd)
```
